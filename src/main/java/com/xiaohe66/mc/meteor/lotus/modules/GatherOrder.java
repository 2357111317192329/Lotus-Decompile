...
public class GatherOrder extends Module {
   ...
   private static final String VERSION = FabricLoader.getInstance()
      .getModContainer("xiaohe66-meteor-lotus")
      .map(container -> container.getMetadata().getVersion().getFriendlyString())
      .orElse("");
   private static final String[] POEMS = new String[]{
      "粉身碎骨浑不怕，要留清白在人间。",
      "咬定青山不放松，立根原在破岩中。",
      "出淤泥而不染，濯清涟而不妖。",
      "不要人夸好颜色，只留清气满乾坤。",
   ...
   private final Setting<Integer> intervalMinutes = this.sgGeneral
      .add(((Builder)((Builder)((Builder)new Builder().name("发送间隔")).description("触发后发送消息的冷却时间（分钟）")).min(1).sliderMax(60).defaultValue(5)).build());
   private final Setting<String> keyword = this.sgGeneral
      .add(
   ...
   private final Setting<Boolean> permanentlyDisabled = this.sgGeneral
      .add(
   ...
   private void onReceiveMessage(ReceiveMessageEvent event) {
      if (this.isActive()) {
         ...
         String keywordValue = (String)this.keyword.get();
         if (!keywordValue.isBlank()) {
            if (text.contains(keywordValue)) {
               long intervalMillis = ((Integer)this.intervalMinutes.get()).intValue() * 60L * 1000L;
               long now = System.currentTimeMillis();
               if (now - this.lastSendTime < intervalMillis) {
                  return;
               }
   ...
   private String resolveMessage() {
      String raw = (String)this.message.get();
      if (raw.contains("{random}")) {
         int index = this.random.nextInt(POEMS.length);
         raw = raw.replace("{random}", POEMS[index]);
      }
   ...
   public boolean isPermanentlyDisabled() {
      return (Boolean)this.permanentlyDisabled.get();
   }
}
