...
public class KitIconRenderer {
    ...
    public void render(GuiGraphicsExtractor drawContext) {
        int scaledWidth = MeteorClient.mc.getWindow().getGuiScaledWidth();
        int scaledHeight = MeteorClient.mc.getWindow().getGuiScaledHeight();
        int hotbarWidth = 182;
        int hotbarHeight = 22;
    ...
    public void render(GuiGraphicsExtractor drawContext, List<Slot> slots) {
        for (Slot slot : slots) {
    ...
    private void drawIcon(GuiGraphicsExtractor drawContext, ItemStack stack, int x, int y) {
        ...
        if (!maxItem.isEmpty()) {
            ...
            drawContext.fakeItem(maxItem, 0, 0);
            matrices.popMatrix();
            matrices.pushMatrix();
            matrices.translate(-2.0F, 0.0F);
            int barTop = 2;
            int barHeight = 12;
            int fillHeight = (int)((double)maxItem.getCount() * (double)1.0F / (double)maxItem.getMaxStackSize() / (double)27.0F * (double)barHeight);
            int barX1 = x + 16;
            int barX2 = x + 17;
            int barY1 = y + barTop;
